...
dat.intakes= dat %>% select(id, Visit, n_intakes_30days) %>%
  pivot_wider(names_from = Visit,
              values_from = n_intakes_30days)
...
ggplot(dat, aes(as.factor(Visit), n_intakes_30days, group=id)) + geom_line(aes(col=id, group=id))+
...
gbtm4_sum = data.frame(gbtm4_sum)
gbtm4_sum$id = rownames(gbtm4_sum)
gbtm4_sum = gbtm4_sum %>% gather(key="group", value="prob", -id)

gbtm4_sum %>% ggplot() + 
  geom_histogram(aes(x = prob, fill = group)) + facet_wrap(~group)
